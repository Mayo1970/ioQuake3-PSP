...
#if !defined(DEDICATED) && !defined(__PSP__)
...
#include <pspkernel.h>
#ifdef PSP_ME_AUDIO
#include "../psp/me/psp_me_audio.h"
#include "../psp/me/psp_me_audio_a5.h"
#endif

// code/sys/sys_psp.c
extern qboolean Sys_PSP_Running( void );
extern void Sys_PSP_ScanBss( void );
extern char *Sys_PSP_ResolveBasePath( const char *argv0 );
extern void Sys_PSP_PrintBootDiagnostics( void );
extern void Sys_PSP_BuildBootCommandLine( char *cmdline, int size );
#endif
...
The PSP builds these three in sys_psp.c instead whenever it has native game
modules, by either route: PSP_NATIVE_GAME_MODULES loads a PRX by UID and gets
its entry points from link-time import stubs (psp/psp_modules.S) because the
platform has no dlsym, and PSP_STATIC_GAME_MODULES - the one that is built,
see cmake/platforms/psp.cmake - links the modules into the EBOOT and just
returns those symbols. See the block comment there.

With neither, these generic versions are used against the Sys_LoadLibrary
stubs in sys_loadlib.h, which return NULL. Nothing calls them, because
vm_cgame/vm_ui are left at the interpreter.
=================
*/
...
#define MINSDL_VERSION \
...
#if defined(__PSP__) && defined(PSP_BOOT_TRACE)
	// FIRST statement that runs: the .bss zeroing measurement is only valid
	// before anything in the engine writes there.
	Sys_PSP_ScanBss( );
#endif
...
#ifdef __PSP__
	// argv[0] alone is not enough: the XMB, psplink and a PBP launcher each
	// pass something different, and a PSP Go has no ms0: (Q3PORT.md 5).
	Sys_SetBinaryPath( Sys_PSP_ResolveBasePath( argc > 0 ? argv[ 0 ] : NULL ) );
#else
...
#ifdef __PSP__
	// The log only exists from CON_Init onwards, and it is the whole
	// observation channel for an XMB run - so the path decision made above
	// gets dumped here, not where it was taken.
	Sys_PSP_PrintBootDiagnostics( );

	// Wii-pattern boot budget (Q3PORT.md 1.4/2): measure free memory and
	// inject com_hunkMegs/zone/sound/etc as boot commands rather than
	// hardcoding constants.
	Sys_PSP_BuildBootCommandLine( commandLine, sizeof( commandLine ) );
#endif
	...
	{
		char meA6Report[ 512 ];
		const int meA6Result = PSP_ME_AudioA6_Start( meA6Report,
			sizeof( meA6Report ) );
		Sys_Print( va( "PSP ME A6 result %d: %s\n", meA6Result,
			meA6Report ) );
	}
	#else
	{
		char meReport[ 512 ];
		const int meResult = PSP_ME_AudioRunMailboxSmoke( meReport,
			sizeof( meReport ) );
		Sys_Print( va( "PSP ME A4 result %d: %s\n", meResult, meReport ) );
	}
	{
		char meA5Report[ 512 ];
		const int meA5Result = PSP_ME_AudioA5_RunSelfTest( meA5Report,
			sizeof( meA5Report ) );
		Sys_Print( va( "PSP ME A5 result %d: %s\n", meA5Result,
			meA5Report ) );
	}
	#endif
#endif
	...
	while( Sys_PSP_Running( ) )
	{
#ifdef PSP_ME_AUDIO
		PSP_ME_AudioPumpRecovery( );
#endif
		Com_Frame( );
	}
	Sys_Exit( 0 );

...
#ifdef __PSP__
// Session 10 step 1a instrumentation. Answers one question before anything is
// built on top of it: is S_PaintChannels actually expensive, or is the main
// thread simply not calling S_Update_ often enough? The underrun counter in
// psp_snd.c cannot tell those apart - it only sees "nobody painted".
// Implemented in code/psp/psp_snd.c; remove both hooks once the answer is in.
void PSP_SndPaintBegin( void );
void PSP_SndPaintEnd( int samples, int activeChannels );

#ifdef PSP_SND_MIX_ASM
void PSP_SndMixMono16Asm( const short *samples, int leftvol, int rightvol,
	portable_samplepair_t *out, int count );
void PSP_SndMixStereo16Asm( const short *samples, int leftvol, int rightvol,
	portable_samplepair_t *out, int count );
qboolean PSP_SndMixAsmSelfTest( void );
#endif

#ifdef PSP_ME_AUDIO_A6
/* A6 handles only ordinary, uncompressed PCM spans.  It seeds the output
 * from the already-painted raw stream and returns false for any job that
 * must remain on the CPU. */
qboolean PSP_ME_AudioA6_TryMix( int startTime, int endTime,
	portable_samplepair_t *seedOutput );
#endif
#endif
...
// Platform headers (code/psp/psp_platform.h) may shrink this; upstream default
// costs MAX_CLIENTS*2+1 == 129 streams * 128 KB == 16.9 MB of .bss.
// Streams above 0 are VoIP only (cl_parse.c CL_PlayVoip, #ifdef USE_VOIP);
// stream 0 carries cinematics and background music.

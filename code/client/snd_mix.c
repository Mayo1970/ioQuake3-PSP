...
/* C owns Quake's linked sndBuffer traversal; the Allegrex kernels operate
   only on contiguous PCM spans and reproduce the existing wrapping adds. */
static void PSP_SndMixMono16Reference( const short *samples, int leftvol,
	int rightvol, portable_samplepair_t *out, int count )
{
	while( count-- > 0 ) {
		const int data = *samples++;
		out->left = (int)( (unsigned int)out->left +
			(unsigned int)( ( data * leftvol ) >> 8 ) );
		out->right = (int)( (unsigned int)out->right +
			(unsigned int)( ( data * rightvol ) >> 8 ) );
		out++;
	}
}

static void PSP_SndMixStereo16Reference( const short *samples, int leftvol,
	int rightvol, portable_samplepair_t *out, int count )
{
	while( count-- > 0 ) {
		const int left = *samples++;
		const int right = *samples++;
		out->left = (int)( (unsigned int)out->left +
			(unsigned int)( ( left * leftvol ) >> 8 ) );
		out->right = (int)( (unsigned int)out->right +
			(unsigned int)( ( right * rightvol ) >> 8 ) );
		out++;
	}
}

static void PSP_SndMixPCM16Asm( const sndBuffer *chunk, int sampleOffset,
	int channels, int leftvol, int rightvol, portable_samplepair_t *out, int count )
{
	while( count > 0 ) {
		int span = ( SND_CHUNK_SIZE - sampleOffset ) / channels;
		...
		if( channels == 1 ) {
			PSP_SndMixMono16Asm( chunk->sndChunk + sampleOffset, leftvol,
				rightvol, out, span );
		} else {
			PSP_SndMixStereo16Asm( chunk->sndChunk + sampleOffset, leftvol,
				rightvol, out, span );
		}

		out += span;
		count -= span;
		sampleOffset += span * channels;

		if( sampleOffset == SND_CHUNK_SIZE && count > 0 ) {
			chunk = chunk->next;
			sampleOffset = 0;
		}
	}
...
static unsigned int PSP_SndMixTestRandom( unsigned int *seed )
{
	*seed = *seed * 1664525u + 1013904223u;
	return *seed;
}

qboolean PSP_SndMixAsmSelfTest( void )
{
	static const int counts[] = { 0, 1, 2, 3, 4, 5, 7, 8, 15, 16, 31 };
	static const int volumes[] = { 0, 1, 255, 65025 };
	short source[ 64 ];
	portable_samplepair_t reference[ 32 ];
	portable_samplepair_t assembled[ 32 ];
	unsigned int seed = 0x713C5A1Du;
	int mode, volume, count, i;

	for( i = 0; i < (int)( sizeof( source ) / sizeof( source[ 0 ] ) ); i++ ) {
		source[ i ] = (short)( PSP_SndMixTestRandom( &seed ) >> 16 );
	}

	for( mode = 0; mode < 2; mode++ ) {
		for( volume = 0; volume < (int)( sizeof( volumes ) / sizeof( volumes[ 0 ] ) ); volume++ ) {
			for( count = 0; count < (int)( sizeof( counts ) / sizeof( counts[ 0 ] ) ); count++ ) {
				const int frames = counts[ count ];

				for( i = 0; i < 32; i++ ) {
					reference[ i ].left = (int)PSP_SndMixTestRandom( &seed );
					reference[ i ].right = (int)PSP_SndMixTestRandom( &seed );
					assembled[ i ] = reference[ i ];
				}

				if( mode == 0 ) {
					PSP_SndMixMono16Reference( source, volumes[ volume ],
						volumes[ ( volume + 1 ) & 3 ], reference, frames );
					PSP_SndMixMono16Asm( source, volumes[ volume ],
						volumes[ ( volume + 1 ) & 3 ], assembled, frames );
				} else {
					PSP_SndMixStereo16Reference( source, volumes[ volume ],
						volumes[ ( volume + 1 ) & 3 ], reference, frames );
					PSP_SndMixStereo16Asm( source, volumes[ volume ],
						volumes[ ( volume + 1 ) & 3 ], assembled, frames );
				}

				for( i = 0; i < 32; i++ ) {
					if( reference[ i ].left != assembled[ i ].left ||
						reference[ i ].right != assembled[ i ].right ) {
						Com_Printf( "PSP audio: mix-asm mismatch (%s, %d frames, lane %d)\n",
							mode ? "stereo" : "mono", frames, i );
						return qfalse;
					}
				}
			}
	...
	Com_Printf( "PSP audio: mix-asm self-test passed\n" );
	return qtrue;
}
...
#ifdef PSP_SND_MIX_ASM
		if( sc->soundChannels == 1 || sc->soundChannels == 2 ) {
			PSP_SndMixPCM16Asm( chunk, sampleOffset, sc->soundChannels,
				leftvol, rightvol, samp, count );
			return;
		}
#endif
		for ( i=0 ; i<count ; i++ ) {
...
//Com_Printf ("%i to %i\n", s_paintedtime, endtime);
	while ( s_paintedtime < endtime ) {
		...
		/* A6 owns only ordinary PCM.  It seeds its output from the raw stream
		 * already mixed above and returns false for unsupported jobs, leaving
		 * the scalar mixer below as the complete fallback. */
		if( PSP_ME_AudioA6_TryMix( s_paintedtime, end, paintbuffer ) ) {

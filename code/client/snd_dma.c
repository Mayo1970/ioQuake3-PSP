...
#ifdef __PSP__
#define PSP_SOUND_PIN_MAX	32
static int pspPinnedSoundCount;

static qboolean S_PSP_ShouldPinSound( const char *name ) {
	const char *leaf;

	if ( !name || strncmp( name, "sound/player/", 13 ) ) {
		return qfalse;
	}
	...
	/* These are the model-specific sounds commonly hit during play. */
	if ( !Q_stricmp( leaf, "jump1.wav" ) ||
		 !Q_stricmp( leaf, "fall1.wav" ) ||
		 !Q_stricmp( leaf, "pain25_1.wav" ) ||
		 !Q_stricmp( leaf, "pain50_1.wav" ) ||
		 !Q_stricmp( leaf, "pain75_1.wav" ) ||
		 !Q_stricmp( leaf, "pain100_1.wav" ) ) {
		return qtrue;
	}

	/* Keep the shared landing sound and normal footsteps resident as well. */
	if ( !Q_stricmp( name, "sound/player/land1.wav" ) ||
		 !Q_stricmp( name, "sound/player/footsteps/step1.wav" ) ||
		 !Q_stricmp( name, "sound/player/footsteps/step2.wav" ) ||
		 !Q_stricmp( name, "sound/player/footsteps/step3.wav" ) ||
		 !Q_stricmp( name, "sound/player/footsteps/step4.wav" ) ) {
		return qtrue;
	}
...
#ifdef __PSP__
	if ( pspPinnedSoundCount < PSP_SOUND_PIN_MAX && S_PSP_ShouldPinSound( name ) ) {
		sfx->pspPinned = qtrue;
		pspPinnedSoundCount++;
	}
#endif
...
#ifdef __PSP__
		Sys_PSP_RenderProfileBegin( PSP_RPROF_SOUND_LAZY_LOAD );
#endif
		S_memoryLoad(sfx);
#ifdef __PSP__
		Sys_PSP_RenderProfileEnd( PSP_RPROF_SOUND_LAZY_LOAD );
#endif
	}
...
#ifdef __PSP__
	{
		int i, active = numLoopChannels;
		// Sample pairs this call will actually paint. Must be sampled BEFORE
		// the call: S_PaintChannels advances s_paintedtime to endtime.
		int todo = (int)( endtime - (unsigned)s_paintedtime );
		...
		PSP_SndPaintBegin();
		Sys_PSP_RenderProfileBegin( PSP_RPROF_SOUND_PAINT );
		S_PaintChannels (endtime);
		Sys_PSP_RenderProfileEnd( PSP_RPROF_SOUND_PAINT );
		PSP_SndPaintEnd( todo, active );
	}
#else
	S_PaintChannels (endtime);
#endif
...
qboolean S_FreeOldestSound( void ) {
...
#ifdef __PSP__
	Sys_PSP_RenderProfileBegin( PSP_RPROF_SOUND_EVICT );
#endif
	...
	/* If every resident sound is protected, fall back instead of looping. */
	if ( used < 0 ) {
		oldest = 0x7fffffff;
		for (i=1 ; i < s_numSfx ; i++) {
			sfx = &s_knownSfx[i];
	...
	if ( used < 0 ) {
		Com_DPrintf( "S_FreeOldestSound: no resident sound to free\n" );
#ifdef __PSP__
		Sys_PSP_RenderProfileEnd( PSP_RPROF_SOUND_EVICT );
#endif
		return qfalse;
	}

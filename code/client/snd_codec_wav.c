...
void *S_WAV_CodecLoad(const char *filename, snd_info_t *info)
{
	...
	unsigned int openUs, headerUs, allocUs, readUs;
#endif

	// Try to open the file
	#ifdef __PSP__
	stageStart = Sys_PSP_RenderProfileNow();
	Sys_PSP_RenderProfileBegin( PSP_RPROF_SOUND_CODEC_OPEN );
	#endif
#ifdef __PSP__
	FS_FOpenFileRead(filename, &file, qfalse);
#else
	FS_FOpenFileRead(filename, &file, qtrue);
#endif
	#ifdef __PSP__
	Sys_PSP_RenderProfileEnd( PSP_RPROF_SOUND_CODEC_OPEN );
	openUs = Sys_PSP_RenderProfileNow() - stageStart;
	#ifdef PSP_FILE_TRACE
	Sys_PSP_FileTraceEvent( PSP_TRACE_SOUND_CODEC_OPEN, openUs, 0 );
	#endif
	#endif
	if(!file)
	{
	...
	#ifdef __PSP__
	stageStart = Sys_PSP_RenderProfileNow();
	Sys_PSP_RenderProfileBegin( PSP_RPROF_SOUND_CODEC_HEADER );
	#endif
	headerOk = S_ReadRIFFHeader(file, info);
	#ifdef __PSP__
	Sys_PSP_RenderProfileEnd( PSP_RPROF_SOUND_CODEC_HEADER );
	headerUs = Sys_PSP_RenderProfileNow() - stageStart;
	#ifdef PSP_FILE_TRACE
	Sys_PSP_FileTraceEvent( PSP_TRACE_SOUND_CODEC_HEADER, headerUs, 0 );
	#endif
	#endif
	if(!headerOk)
	{
	...
	#ifdef __PSP__
	stageStart = Sys_PSP_RenderProfileNow();
	Sys_PSP_RenderProfileBegin( PSP_RPROF_SOUND_CODEC_ALLOC );
	#endif
	buffer = Hunk_AllocateTempMemory(info->size);
	#ifdef __PSP__
	Sys_PSP_RenderProfileEnd( PSP_RPROF_SOUND_CODEC_ALLOC );
	allocUs = Sys_PSP_RenderProfileNow() - stageStart;
	#ifdef PSP_FILE_TRACE
	Sys_PSP_FileTraceEvent( PSP_TRACE_SOUND_CODEC_ALLOC, allocUs,
	...
	#ifdef __PSP__
	stageStart = Sys_PSP_RenderProfileNow();
	Sys_PSP_RenderProfileBegin( PSP_RPROF_SOUND_CODEC_READ );
	#endif
	bytesRead = FS_Read(buffer, info->size, file);
	S_ByteSwapRawSamples(info->samples, info->width, info->channels, (byte *)buffer);
	#ifdef __PSP__
	Sys_PSP_RenderProfileEnd( PSP_RPROF_SOUND_CODEC_READ );
	readUs = Sys_PSP_RenderProfileNow() - stageStart;
	#ifdef PSP_FILE_TRACE
	Sys_PSP_FileTraceEvent( PSP_TRACE_SOUND_CODEC_READ, readUs,
		(unsigned int)bytesRead );
	#endif
	#endif
#ifdef __PSP__
#ifndef PSP_FILE_TRACE
	Com_Printf( "PSP sound codec: %s open=%u header=%u alloc=%u read=%u us bytes=%d/%d\n",
		filename, openUs, headerUs, allocUs, readUs, bytesRead, info->size );
#endif
#endif

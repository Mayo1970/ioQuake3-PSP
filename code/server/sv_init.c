...
#ifdef __PSP__
	// DIAGNOSTIC - Session 7 map-load failure. See code/psp/psp_platform.h.
	Sys_PSP_HeapReport("SV_SpawnServer entry");
#endif
...
//	Cvar_Set( "nextmap", va("map %s", server) );
	...
	// A local map does not change the search path. On PSP, refresh only the
	// feed-dependent pure checksums instead of re-indexing every PK3 archive.
	sv.checksumFeed = ( ((unsigned int)rand() << 16) ^ (unsigned int)rand() ) ^ Com_Milliseconds();
	#ifdef __PSP__
	FS_UpdateChecksumFeed( sv.checksumFeed );
	#else
	FS_Restart( sv.checksumFeed );
	#endif

#ifdef __PSP__
	Sys_PSP_HeapReport("after Hunk_Clear + checksum refresh");
#endif

	CM_LoadMap( va("maps/%s.bsp", server), qfalse, &checksum );

#ifdef __PSP__
	Sys_PSP_HeapReport("after CM_LoadMap");
#endif
...
#ifdef __PSP__
	Sys_PSP_HeapReport("before SV_InitGameProgs");
#endif

	SV_InitGameProgs();

#ifdef __PSP__
	Sys_PSP_HeapReport("after SV_InitGameProgs");
#endif

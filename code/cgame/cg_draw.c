...
void CG_DrawActive( stereoFrame_t stereoView ) {
	...
	// draw 3D view
	#ifdef __PSP__
	Sys_PSP_RenderProfileBegin( PSP_RPROF_CGAME_SCENE );
	#endif
	trap_R_RenderScene( &cg.refdef );
	#ifdef __PSP__
	Sys_PSP_RenderProfileEnd( PSP_RPROF_CGAME_SCENE );
	#endif

	// draw status bar and other floating elements
	#ifdef __PSP__
	Sys_PSP_RenderProfileBegin( PSP_RPROF_CGAME_HUD );
	#endif
 	CG_Draw2D(stereoView);
	#ifdef __PSP__
	Sys_PSP_RenderProfileEnd( PSP_RPROF_CGAME_HUD );
	#endif
}

...
#endif
#ifndef MIN_COMHUNKMEGS
#define MIN_COMHUNKMEGS		DEF_COMHUNKMEGS
#endif
#ifndef DEF_COMZONEMEGS
...
#ifdef PSP_BOOT_TRACE
	Sys_Print( va( "boot: smallzone calloc done, ptr %p\n", (void *)smallzone ) );
#endif
	if ( !smallzone ) {
...
#ifdef PSP_BOOT_TRACE
	Sys_Print( va( "boot: zone calloc enter, %i bytes\n", s_zoneTotal ) );
#endif
	mainzone = calloc( s_zoneTotal, 1 );
	BOOT_TRACE( "zone calloc done" );
	if ( !mainzone ) {
		Com_Error( ERR_FATAL, "Zone data failed to allocate %i megs", s_zoneTotal / (1024*1024) );
	}
	Z_ClearZone( mainzone, s_zoneTotal );
	BOOT_TRACE( "zone cleared" );
...
#ifndef DEDICATED
#ifdef __PSP__
char	cl_cdkey[34] = PSP_CDKEY_SEED;
#else
char	cl_cdkey[34] = "                                ";
#endif
#else
...
void Com_AppendCDKey( const char *filename ) {
	...
	if (CL_CDKeyValidate(buffer, NULL)) {
#ifdef __PSP__
		/* Not strcat: the slot is pre-seeded above, so appending would run
		   past cl_cdkey[33] on a second FS_Startup (game restart). */
...
void Com_Init( char *commandLine ) {
	...
	BOOT_TRACE( "setjmp returned" );

	// Clear queues
	Com_Memset( &eventQueue[ 0 ], 0, MAX_QUEUED_EVENTS * sizeof( sysEvent_t ) );
	BOOT_TRACE( "queues cleared" );
	...
	Com_InitPushEvent();
	BOOT_TRACE( "pushevent" );

	Com_InitSmallZoneMemory();
	BOOT_TRACE( "smallzone" );
	Cvar_Init ();
	BOOT_TRACE( "cvar" );
...
void Com_Frame( void ) {
	...
	#if defined( __PSP__ ) && defined( PSP_FILE_TRACE )
	Sys_PSP_FileTraceFrameBegin();
	#endif
	if ( setjmp (abortframe) ) {
...
#ifdef __PSP__
	Sys_PSP_ZoneBegin( PSP_ZONE_SVFRAME );
#endif
...
#ifdef __PSP__
	// Last statement in the frame: the window this closes is wall clock, so
	// every zone measured above is inside it and "other" is a real remainder.
	Sys_PSP_FrameMark( );
#endif

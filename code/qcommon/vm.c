...
intptr_t QDECL VM_DllSyscall( intptr_t arg, ... ) {
  ...
  int i;
  va_list ap;
#if defined(__PSP__) && defined(PSP_RENDER_PROFILE)
  unsigned int pspCgameSyscallStart = 0;
  const qboolean pspCgameSyscall = currentVM &&
    !Q_stricmp( currentVM->name, "cgame" );
#endif
...
#if defined(__PSP__) && defined(PSP_RENDER_PROFILE)
  if( pspCgameSyscall ) {
    pspCgameSyscallStart = Sys_PSP_RenderProfileCGameSyscallBegin( (int)arg );
  }
#endif

  arg = currentVM->systemCall( args );

#if defined(__PSP__) && defined(PSP_RENDER_PROFILE)
  if( pspCgameSyscall ) {
    Sys_PSP_RenderProfileCGameSyscallEnd( (int)args[0], pspCgameSyscallStart );
  }
#endif
...
vm_t *VM_Create( const char *module, intptr_t (*systemCalls)(intptr_t *), 
	...
	/*
	 PSP: cgame and ui are linked into the engine binary (cmake/basegame.cmake),
	 so there is no file for FS_FindVM to find and no VMI_NATIVE to return -
	 it only reports native when it sees an actual DLL_EXT file in a directory
	 searchpath (qcommon/files.c:1468-1492). Ask sys_psp.c directly instead.

	 Falling through on failure is deliberate and is the whole fallback story:
	 Sys_LoadGameDll leaves *entryPoint untouched unless it succeeded, so
	 qagame - which has no blob - and any module whose blob is missing take the
	 ordinary FS_FindVM path below and end up on the .qvm in the pk3.
	*/
	if ( interpret == VMI_NATIVE ) {
		...
		Com_Printf( "No built-in module '%s', trying the VM\n", module );
	}
#endif

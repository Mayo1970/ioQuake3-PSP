...
typedef struct {
	...
	int				*checksumList;				// feed followed by the file CRCs
	int				checksumCount;				// number of entries in checksumList
...
static qboolean FS_PSP_SharedPakHandleBusy( const pack_t *pak ) {
	...
	for ( i = 1; i < MAX_FILE_HANDLES; i++ ) {
		if ( fsh[i].zipFile &&
				!fsh[i].handleFiles.unique &&
				fsh[i].handleFiles.file.z == pak->handle ) {
			return qtrue;
		}
	}
...
long FS_FOpenFileReadDir(const char *filename, searchpath_t *search, fileHandle_t *file, qboolean uniqueFILE, qboolean unpure)
{
	...
	if(file == NULL)
	{
		...
		#ifdef PSP_STUTTER_TRACE
			Sys_PSP_StutterTraceSetPack( search->pack->pakFilename );
			Sys_PSP_StutterTraceRecord( PSP_STUTTER_PHASE_PACK_HASH,
				Sys_PSP_RenderProfileNow() - traceStart, -1, -1,
				(int)hash, 0, 0, 0, 0, search->pack->pakFilename );
		#endif
		...
		else if(search->dir)
		{
			...
			if(filep)
			{
				...
				{
				#ifdef PSP_STUTTER_TRACE
					Sys_PSP_StutterTraceLookupSource( PSP_STUTTER_SOURCE_LOOSE );
				#endif
					return len;
				}
				else
		...
		if(search->pack->hashTable[hash])
		{
			...
			do
			{
				...
				if(!FS_FilenameCompare(pakFile->name, filename))
				{
					...
					qboolean openUnique = uniqueFILE;
#ifdef __PSP__
					if(!openUnique && FS_PSP_SharedPakHandleBusy(pak))
					{
						openUnique = qtrue;
						Com_DPrintf("FS_FOpenFileRead: shared pak handle busy for '%s'; using unique handle for '%s'\n",
								pak->pakFilename, filename);
					}
#endif
					fsh[*file].handleFiles.unique = openUnique;
					#ifdef PSP_STUTTER_TRACE
					fsh[*file].tracePackHash =
						Sys_PSP_StutterTraceHashPath( pak->pakFilename );
					Sys_PSP_StutterTraceSetContext( filename,
						fsh[*file].tracePackHash );
					#endif
							...
							*/
							{
								...
								for(h = 1; h < MAX_FILE_HANDLES; h++)
								{
									if(fsh[h].handleFiles.file.o || fsh[h].handleFiles.file.z)
										openHandles++;
								}

								Com_Printf("PSP: unzOpen failed for '%s' while opening '%s'\n",
										pak->pakFilename, filename);
								Com_Printf("PSP: errno %d, %d/%d engine file handles in use\n",
										savedErrno, openHandles, MAX_FILE_HANDLES);

								Sys_PSP_HeapReport("unzOpen failure");

								probe = Sys_FOpen(pak->pakFilename, "rb");
								Com_Printf("PSP: direct fopen retry %s (errno %d)\n",
										probe ? "SUCCEEDED - not the file system, the heap"
										      : "FAILED   - file system or descriptor limit",
										probe ? 0 : errno);
								if(probe)
									fclose(probe);
							}
					...
					#ifdef PSP_STUTTER_TRACE
					Sys_PSP_StutterTraceSetPhase( PSP_STUTTER_PHASE_UNZ_OFFSET );
					traceStart = Sys_PSP_RenderProfileNow();
					#ifdef __PSP__
					traceResult = unzSetOffsetFast(fsh[*file].handleFiles.file.z,
						pakFile->pos, pakFile->localPos, pakFile->flags,
						pakFile->compressionMethod, pakFile->crc,
						pakFile->compressedLen, pakFile->len,
						pakFile->nameLen, pakFile->extraLen);
					#else
					traceResult = unzSetOffset(fsh[*file].handleFiles.file.z, pakFile->pos);
					#endif
					Sys_PSP_StutterTraceRecord( PSP_STUTTER_PHASE_UNZ_OFFSET,
						Sys_PSP_RenderProfileNow() - traceStart, *file, -1,
						(int)pakFile->pos, SEEK_SET, 0, traceResult,
						PSP_STUTTER_FLAG_PARENT, pak->pakFilename );
					#else
					#ifdef __PSP__
					unzSetOffsetFast(fsh[*file].handleFiles.file.z,
						pakFile->pos, pakFile->localPos, pakFile->flags,
						pakFile->compressionMethod, pakFile->crc,
						pakFile->compressedLen, pakFile->len,
						pakFile->nameLen, pakFile->extraLen);
					...
					#ifdef PSP_STUTTER_TRACE
					Sys_PSP_StutterTraceSetPhase( PSP_STUTTER_PHASE_LOCAL_HEADER );
					traceStart = Sys_PSP_RenderProfileNow();
					traceResult = unzOpenCurrentFile(fsh[*file].handleFiles.file.z);
					Sys_PSP_StutterTraceRecord( PSP_STUTTER_PHASE_LOCAL_HEADER,
						Sys_PSP_RenderProfileNow() - traceStart, *file, -1,
						(int)pakFile->pos, SEEK_SET, 0, traceResult,
						PSP_STUTTER_FLAG_PARENT, pak->pakFilename );
...
long FS_FOpenFileRead(const char *filename, fileHandle_t *file, qboolean uniqueFILE)
{
...
#ifdef PSP_STUTTER_TRACE
	Sys_PSP_StutterTraceLookupBegin( filename );
#endif
	...
	if(file)
	{
		*file = 0;
	#ifdef PSP_STUTTER_TRACE
		Sys_PSP_StutterTraceLookupEnd( -1 );
	#endif
		return -1;
	}
	else
	...
	#ifdef PSP_STUTTER_TRACE
		Sys_PSP_StutterTraceLookupEnd( 0 );
	#endif
		return 0;
	}
}
...
#ifdef PSP_STUTTER_TRACE
		Sys_PSP_StutterTraceSetContext( fsh[f].name,
			fsh[f].tracePackHash );
		Sys_PSP_StutterTraceSetPhase( PSP_STUTTER_PHASE_REFILL_READ );
		zipRead = unzReadCurrentFile(fsh[f].handleFiles.file.z, buffer, len);
		Sys_PSP_StutterTraceSetPhase( PSP_STUTTER_PHASE_NONE );
...
static pack_t *FS_LoadZipFileLegacy(const char *zipfile, const char *basename)
{
	...
	pack->checksumList = fs_headerLongs;
	unzGoToFirstFile(uf);
	...
	pack->checksumCount = fs_numHeaderLongs;
...
} fsZipDirectoryEntry_t;

static unsigned int FS_ZipLittleShort( const byte *p )
{
	return (unsigned int)p[ 0 ] | ( (unsigned int)p[ 1 ] << 8 );
}

static unsigned long FS_ZipLittleLong( const byte *p )
{
...
static qboolean FS_NextZipDirectoryEntry( const byte *directory,
	unsigned long directorySize, unsigned long centralOffset,
	unsigned long *cursor, fsZipDirectoryEntry_t *entry )
{
	const byte *header;
	...
	if ( !directory || !cursor || !entry || *cursor > directorySize ||
		directorySize - *cursor < 46 ) {
		return qfalse;
	}

	header = directory + *cursor;
	if ( FS_ZipLittleLong( header ) != 0x02014b50 ) {
		return qfalse;
	}

	nameLength = FS_ZipLittleShort( header + 28 );
	extraLength = FS_ZipLittleShort( header + 30 );
	commentLength = FS_ZipLittleShort( header + 32 );
	recordSize = 46UL + nameLength + extraLength + commentLength;
	if ( recordSize > directorySize - *cursor || nameLength >= MAX_ZPATH ) {
		return qfalse;
	}

	entry->name = header + 46;
	entry->nameLength = nameLength;
	entry->pos = centralOffset + *cursor;
	entry->crc = FS_ZipLittleLong( header + 16 );
	entry->localPos = FS_ZipLittleLong( header + 42 );
	entry->compressedSize = FS_ZipLittleLong( header + 20 );
	entry->uncompressedSize = FS_ZipLittleLong( header + 24 );
	entry->flags = (unsigned short)FS_ZipLittleShort( header + 8 );
	entry->compressionMethod = (unsigned short)FS_ZipLittleShort( header + 10 );
	entry->extraLength = (unsigned short)extraLength;
	*cursor += recordSize;
	return qtrue;
}
...
static pack_t *FS_LoadZipFile( const char *zipfile, const char *basename )
{
	...
	char *namePtr;
	byte *directory = NULL;
	unsigned long directorySize, centralOffset, cursor;
	fsZipDirectoryEntry_t entry;
	...
	directorySize = unzGetCentralDirectorySize( uf );
	centralOffset = unzGetCentralDirectoryOffset( uf );
	if ( directorySize > 0x7fffffffUL ) {
		Com_Printf( "FS_LoadZipFile: central directory too large for %s\n", zipfile );
		unzClose( uf );
		return NULL;
	}
	if ( directorySize > 0 ) {
		directory = Z_Malloc( (int)directorySize );
		if ( unzReadCentralDirectory( uf, directory, directorySize ) != UNZ_OK ) {
			goto zipError;
		}
	}

	/* First pass: calculate the exact name arena from the RAM image. */
	len = 0;
	cursor = 0;
	for ( i = 0; i < (int)gi.number_entry; i++ ) {
		if ( !FS_NextZipDirectoryEntry( directory, directorySize,
			centralOffset, &cursor, &entry ) ||
			entry.nameLength > (unsigned int)( 0x7fffffff - len - 1 ) ) {
			Com_Printf( "FS_LoadZipFile: malformed central directory in %s\n", zipfile );
			goto zipError;
		}
		len += (int)entry.nameLength + 1;
	}
	if ( cursor != directorySize ) {
		Com_Printf( "FS_LoadZipFile: central directory length mismatch in %s\n", zipfile );
		goto zipError;
	}

	buildBuffer = Z_Malloc( (int)gi.number_entry * sizeof( fileInPack_t ) + len );
	namePtr = ((char *)buildBuffer) + (int)gi.number_entry * sizeof( fileInPack_t );
	fs_headerLongs = Z_Malloc( ( (int)gi.number_entry + 1 ) * sizeof( int ) );
	fs_headerLongs[ fs_numHeaderLongs++ ] = LittleLong( fs_checksumFeed );
	...
	/* Second pass: build the existing hash/CRC index from the RAM image. */
	cursor = 0;
	for ( i = 0; i < (int)gi.number_entry; i++ ) {
		if ( !FS_NextZipDirectoryEntry( directory, directorySize,
			centralOffset, &cursor, &entry ) ) {
			Com_Printf( "FS_LoadZipFile: central directory changed in %s\n", zipfile );
			goto zipError;
		}
		...
		buildBuffer[ i ].name = namePtr;
		Com_Memcpy( namePtr, entry.name, entry.nameLength );
		namePtr[ entry.nameLength ] = '\0';
		Q_strlwr( namePtr );
		hash = FS_HashFileName( namePtr, pack->hashSize );
		namePtr += entry.nameLength + 1;
		buildBuffer[ i ].pos = entry.pos;
		buildBuffer[ i ].len = entry.uncompressedSize;
		#ifdef __PSP__
		buildBuffer[ i ].localPos = entry.localPos;
		buildBuffer[ i ].crc = entry.crc;
		buildBuffer[ i ].compressedLen = entry.compressedSize;
		buildBuffer[ i ].flags = entry.flags;
		buildBuffer[ i ].compressionMethod = entry.compressionMethod;
		buildBuffer[ i ].nameLen = (unsigned short)entry.nameLength;
		buildBuffer[ i ].extraLen = entry.extraLength;
		#endif
		buildBuffer[ i ].next = pack->hashTable[ hash ];
		pack->hashTable[ hash ] = &buildBuffer[ i ];
	}

	pack->checksum = Com_BlockChecksum( &fs_headerLongs[ 1 ],
		sizeof( *fs_headerLongs ) * ( fs_numHeaderLongs - 1 ) );
	pack->pure_checksum = Com_BlockChecksum( fs_headerLongs,
...
zipError:
	...
	if ( pack ) {
		...
		if ( pack->checksumList ) {
			Z_Free( pack->checksumList );
		}
		if ( pack->buildBuffer ) {
...
void FS_UpdateChecksumFeed( int checksumFeed ) {
	...
	for ( search = fs_searchpaths; search; search = search->next ) {
		...
		if ( !pack || !pack->checksumList || pack->checksumCount <= 0 ) {
			continue;
		}

		pack->checksumList[ 0 ] = LittleLong( checksumFeed );
		pack->pure_checksum = LittleLong( Com_BlockChecksum(
			pack->checksumList,
			pack->checksumCount * (int)sizeof( *pack->checksumList ) ) );
	}
}
